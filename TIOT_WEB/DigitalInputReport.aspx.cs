...
namespace TIOT_WEB
{
    public partial class DigitalInputReport : System.Web.UI.Page
    {
        ...
        public void gvdBind(int ObjectSensorID, DateTime StartDate, DateTime EndDate)
        {
            try
            {
                List<DigitalInputModel> Li = obj.getDigitalInputReport(ObjectSensorID, StartDate, EndDate);
                if (Li.Count > 0)
        ...
        public void ddlObjectSensorbind(int objectId)
        {
            try
            {
                List<ObjectSensorIDName> list = cObj.getObjectSensorDinList(objectId);
                if (list.Count > 0)
