...
namespace TIOT_WEB
{
    public partial class EventAlertReport : System.Web.UI.Page
    {
        ...
        protected void btnGetReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                gvdBind(Convert.ToInt32(ddlobjectSensor.SelectedValue), Startdate, Enddate);
                allowStaticMethods("staticMethod();gridtoJson('gvdReport');gridhtml('#gvdReport','" + ddlobjectSensor.SelectedItem.Text + "','" + ddlobject.SelectedItem.Text + "','" + Startdate + "','" + Enddate + "');");
            }
            catch (Exception)
            { BindingClass.ExceptionAlertScriptManager(this.Page, this.GetType()); }
        }
        ...
        public void ddlObjectSensorbind(int objectId)
        {
            try
            {
                List<ObjectSensorIDName> list = cObj.getObjectSensorDinAndSerialSensor(objectId);
                if (list.Count > 0)
        ...
        public void gvdBind(int ObjectSensorID, DateTime StartDate, DateTime EndDate)
        {
            try
            {
                List<IndividualSensorModel> Li = obj.getEventLogReport(ObjectSensorID, StartDate, DateTime.Now);
                if (Li.Count > 0)
