...
namespace TIOT_WEB
{
    public partial class TemperatureValueReport : System.Web.UI.Page
    {
        ...
        protected void btnGetReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                allowStaticMethods("staticMethod();gridtoJson('gvdReport');gridhtml('#gvdReport','" + ddlobjectSensor.SelectedItem.Text + " Temperature','" + ddlobject.SelectedItem.Text + "','" + Startdate + "','" + Enddate + "');");
            }
            catch (Exception)
            { BindingClass.ExceptionAlertScriptManager(this.Page, this.GetType()); }
        }
        ...
        public void ddlObjectSensorbind(int objectId)
        {
            try
            {
                List<ObjectSensorIDName> list = cObj.getObjectSensorTempValList(objectId);
                if (list.Count > 0)
