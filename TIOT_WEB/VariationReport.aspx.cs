...
namespace TIOT_WEB
{
    public partial class VariationReport : System.Web.UI.Page
    {
        ...
        public void ddlObjectSensorbind(int objectId)
        {
            try
            {
                List<ObjectSensorIDName> list = cObj.getEventConfiguredSensor(objectId);
                if (list.Count > 0)
        ...
        public void gvdBind(int ObjectSensorID, DateTime StartDate, DateTime EndDate)
        {
            try
            {
                List<SensorVariationModel> Li = obj.getSensorVariationReport(ObjectSensorID, StartDate, EndDate);
                if (Li.Count > 0)
